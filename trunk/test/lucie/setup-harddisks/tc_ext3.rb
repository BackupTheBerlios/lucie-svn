#
# $Id: tc_specification.rb 670 2005-06-02 08:26:55Z takamiya $
#
# Author::   Yoshiaki Sakae (mailto:sakae@is.titech.ac.jp)
# Revision:: $LastChangedRevision: 670 $
# License::  GPL2

$LOAD_PATH.unshift 'trunk/lib'

require 'lucie/setup-harddisks/command-line-options'
require 'lucie/setup-harddisks/ext3'
require 'test/unit'

include Lucie::SetupHarddisks

class TC_Ext3 < Test::Unit::TestCase
  public
  def setup
    $commandline_options = CommandLineOptions.instance
    @fs = Ext3.new
    @fs.format_options += [ "-n", "-q", "-v"]
    @fs.mount_options += [ "-v", "-r", "-w", "-o", "default,nosuid" ]
  end
  
  public
  def test_name
    assert_equal( "ext3", @fs.fs_type )
  end
  
  public
  def test_check_programs
    assert_equal( "mke2fs", @fs.format_program )
    assert_equal( "mount -t ext3", @fs.mount_program )
  end
  
  public
  def test_fsck_enabled
    assert_equal( true, @fs.fsck_enabled) 
  end
  
  public
  def test_set_options
    assert_equal( [ "-j", "-n", "-q", "-v"], @fs.format_options )
    assert_equal( [ "-v", "-r", "-w", "-o", "default,nosuid" ], @fs.mount_options )
  end
  
  public
  def test_format_program
    assert_equal( "mke2fs -j -n -q -v -L VOL /dev/hda1", @fs.dump_format( "/dev/hda1", "VOL" ) )
  end
  
  public
  def test_mount_program
    assert_equal( "mount -t ext3 -v -r -w -o default,nosuid /dev/hda1 /", @fs.dump_mount( "/dev/hda1", "/" ) )
  end

  public
  def test_mount_program_with_label
    assert_equal( "mount -t ext3 -v -r -w -o default,nosuid -L BOOT /", @fs.dump_mount_with_label( "BOOT", "/" ) )
  end

end

### Local variables:
### mode: Ruby
### indent-tabs-mode: nil
### End: