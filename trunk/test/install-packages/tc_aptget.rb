...
class TC_AptGet < Test::Unit::TestCase
  ...
  def test_clean_dryrun
    flexstub( STDOUT, 'STDOUT_MOCK' ).should_receive( :puts ).with( " ENV{ ``LC_ALL'' => ``C'' } chroot /tmp/target apt-get clean" ).once.ordered
  ...
  def test_install_dryrun
    flexstub( STDOUT, 'STDOUT_MOCK' ).should_receive( :puts ).with( %{ ENV{ ``LC_ALL'' => ``C'' } chroot /tmp/target apt-get -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" --fix-missing install PACKAGE_A PACKAGE_B PACKAGE_C} ).once.ordered
    flexstub( STDOUT, 'STDOUT_MOCK' ).should_receive( :puts ).with( " ENV{ ``LC_ALL'' => ``C'' } chroot /tmp/target apt-get clean" ).once.ordered
  ...
  def test_remove_dryrun
    flexstub( STDOUT, 'STDOUT_MOCK' ).should_receive( :puts ).with( %{ ENV{ ``LC_ALL'' => ``C'' } chroot /tmp/target apt-get -y -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" --purge remove PACKAGE_A PACKAGE_B PACKAGE_C} ).once.ordered
