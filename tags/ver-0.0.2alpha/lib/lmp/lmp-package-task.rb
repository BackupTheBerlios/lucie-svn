...
module Rake
  ...
  #
  # PackageTask によって生成される Rake ターゲットに加え、
  # LMPPackageTask は次のタスクを生成する。
  #
  # [<b>"<em>package_dir</em>/<em>name</em>-<em>version</em>.deb"</b>]
  #  与えられた名前とバージョンを元に LMP パッケージを作成する
  #
  # LMP の Specification を用いた場合の使用例:
  #
  ...
  class LMPPackageTask < PackageTask
    # LMP パッケージのメタデータを含むスペック。パッケージ名、バージョン 
    # および package_files は自動的に LMP スペックから決定されるため、明  
    # 示的に渡す必要は無い。
